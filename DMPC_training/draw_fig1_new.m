function [NIError,State,R_State]=draw_fig1_new(ANN,ANN_x,ANN_u,ConHor_len,state_bound,NIError,State,R_State,Obstacle,k,Iterations_num)
...
while(timesteps<ConHor_len)
    ...
    u1=ANN{1}.NetworkOut+ANN_u{1}.NetworkOut+1*ANN_x{1}.NetworkOut;
    u2=ANN{2}.NetworkOut+ANN_u{2}.NetworkOut+1*ANN_x{2}.NetworkOut;
...
% % % %% plot path
     ...
     plot(X2(1,:),X2(2,:),'r');
     hold on;
     rectangle('position',[Obstacle(1)-0.1,Obstacle(2)-0.1,0.1*2,0.1*2],'curvature',[1,1],'edgecolor','[0,0,0.5]','linestyle','-','facecolor','b');
     title('Trajectory');
    %%
     figure;
     subplot(2,1,1);
