...
for k=1:ConHor_len:Iterations_num   % Loop over control horizon
 ...
 %% ANN output        
            ...
            [DB_x{i}]=B_neww(PresentState{i},Obstacle);
 ...
 %% Update ANN and CNN
       ...
       %% Determine whether learning is successful whether learning is successful
        if(timesteps==PreHor_len)  %% success
            [NIError,State,R_State]=evaluate(ANN,ANN_x,ANN_u,ConHor_len,state_bound,NIError,State,R_State,Obstacle,k,Iterations_num);
            f=0;
        else                       %% fail
            f=f+1;
        end
...
save('trained_model.mat', 'ANN', 'ANN_u', 'ANN_x', 'CNN', 'CNN_x', 'state_bound', 'J', 'Theorem4_1_roboT', 'Theorem4_2_roboT');
% save('final_state.mat', 'State0', 'R_State0', 'NIError0');
% save('final_trajectory.mat', 'Future_x');
% save('final_R_State.mat', 'R_State');
