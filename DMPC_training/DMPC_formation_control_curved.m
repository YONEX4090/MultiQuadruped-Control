...
% leader neighborhood
leader_neighbour = [1 0 0]; % neighbor-indicator vector
...
q_next_virtual = [p1_0 p2_0 p3_0]; % initialise virtual followers
virtual_dq_k = zeros(size(q_next_k));
...
p_obs = p_l(1:2, obstacle_encounter_time) + [0; 1];
...
for j=1:length(k)
    ...
    for i=1:mrs_size
        ...
        if (1)
            ...
            h_i_dis = -((p_jdot_database*adjacency(i, :)')...
                          + leader_neighbour(i)*dp_l(1:dim_state-1, j));
            ...
            threeD_rotation = R_theta_func(measured_q);
            global_to_local = threeD_rotation(1:2, 1:2);
            local_formation_error = (global_to_local')*global_formation_error;
            formation_error(:, j, i) = full(local_formation_error);
            ...
            formation_stage_cost = (formation_error_next')*Q_e*formation_error_next;
            for r=1:M
                formation_error_next = dt_H_e*formation_error_next...
                                        + dt_G_e*formation_sys_opt_params(1:dim_state-1);
                running_cartesian_velocity = g_i_dis\((-h_i_dis)...
                                + formation_sys_opt_params((r-1)*(dim_state-1)+1:r*(dim_state-1)));
                running_velocity_magnitude = sqrt((running_cartesian_velocity')*running_cartesian_velocity);

                if (r == 1)
                    control_rate = formation_sys_opt_params(1:(dim_state-1))...
                                    - formation_sys_opt_output(1:(dim_state-1), i);
                    formation_stage_cost = formation_stage_cost... 
                                           + 0.5*(control_rate')*S_e*control_rate;
                    formation_stage_cost = formation_stage_cost...
                                            + (formation_error_next')*Q_e*formation_error_next;
                    velocity_error = (running_velocity_magnitude - consensus_velocity_magnitude);
                    formation_stage_cost = formation_stage_cost...
                                                    + penalise_vel*(velocity_error^2);
                elseif (r == M)
                    formation_stage_cost = formation_stage_cost...
                                      + form_beta_val*(formation_error_next')*P_e*formation_error_next;
                else
            ...
            % Define NLP problem
            formation_sys_objective = formation_stage_cost;
            formation_sys_constraint = [h_CBF; auxiliary_input];
            formation_sys_nlp = struct('x', formation_sys_opt_params,...
                                       'f', formation_sys_objective,...
                                       'g', formation_sys_constraint);
        ...
        % uncomment to evaluate dynamics controller
        % desired_y = p_l(1:dim_state-1, j) + Delta(:, i);
        % desired_y_rate = dp_l(1:dim_state-1, j);
        % desired_velocity = full(obtain_vel_func(measured_q(3))*desired_y_rate);
        % unfiltered_desired_y_accel = (desired_y_rate - desired_y_rate_history(:, i))/dt;
        % desired_y_accel_history(:, i) = smoothing_coeff*unfiltered_desired_y_accel...
        %                                 + (1- smoothing_coeff)*desired_y_accel_history(:, i);
        % desired_y_accel = desired_y_accel_history(:, i);
        %%{
        if(1)
            diffeomorphism_k = aux_state_func(x_state_k, desired_y, desired_y_rate);
            v_k = convert_eta_to_v_func(x_state_k, desired_y, desired_y_rate,...
                                        desired_y_accel, dynamics_sys_opt_pars(1:dim_state-1));
            
            tau_k = calculate_torque_func(x_state_k, v_k);
            estimate_state_f = state_solver('x0', x_state_k, 'p', tau_k);
            estimate_state_k = full(estimate_state_f.xf);
            h_CBF_q = cbf_constr_func(C_CBF*x_state_k, C_CBF*estimate_state_k, dynamics_sys_opt_pars(end));

            diffeomorphism_k_next = diffeomorphism_k;
            trajectory_stage_cost = (diffeomorphism_k_next')*Q_0*diffeomorphism_k_next;
            for z=1:N
                diffeomorphism_k_next = dt_H_0*diffeomorphism_k_next...
                                        + dt_G_0*dynamics_sys_opt_pars((z-1)*(dim_state-1)+1:z*(dim_state-1));
                if (z == N)
                    trajectory_stage_cost = trajectory_stage_cost...
                                      + dyn_beta_val*(diffeomorphism_k_next')*P_0...
                                        *diffeomorphism_k_next;
                else
                    trajectory_stage_cost = trajectory_stage_cost + (diffeomorphism_k_next')*Q_0...
                                        *diffeomorphism_k_next;
                end
            end

            dynamics_sys_objective = trajectory_stage_cost...
                            +  0.5*(dynamics_sys_opt_pars(1:N*(dim_state-1))')*R_horizon...
                                *dynamics_sys_opt_pars(1:N*(dim_state-1))...
                            + 0.5*dyn_CBF_weight...
                                *((dynamics_sys_opt_pars(end) - desired_gamma_CBF2)^2);
            dynamics_sys_constraint = [h_CBF_q; tau_k];
            dynamics_sys_nlp = struct('x', dynamics_sys_opt_pars, 'f',...
                                    dynamics_sys_objective, 'g', dynamics_sys_constraint);
            
            % Create an NLP solver
            dynamics_sys_nlpsolve = nlpsol('solver', 'ipopt', dynamics_sys_nlp, opts);
            u_ic_horizon_variables = dynamics_sys_nlpsolve('x0', dynamics_sys_opt_output(:, i),...
                                                 'lbx', dynamics_lbx, 'ubx', dynamics_ubx,...
                                                 'lbg', dynamics_lbg, 'ubg', dynamics_ubg);
            dynamics_sys_opt_output(:, i) = full(u_ic_horizon_variables.x);
            optimal_eta = dynamics_sys_opt_output(1:dim_state-1, i);
            optimal_aux_in = convert_eta_to_v_func(x_state_k, desired_y, desired_y_rate,...
                                                desired_y_accel, optimal_eta);
            tau_i(:, i) = full(calculate_torque_func(x_state_k, optimal_aux_in));
        end
        %}
        ...
        v_i(:, i) = vel_forward*wheel_vel_i(:, i);
        virtual_dq_k(:, i) = [desired_y_rate; des_w_i];
        desired_y_rate_history(:, i) = desired_y_rate;
...
legend('show', 'Location', 'northeast');
legend('Robot_1', 'Robot_2', 'Robot_3', 'Leader');
box on
grid on
grid minor
set(gca, 'fontsize', 12);
...
legend('show', 'Location', 'northeast');
legend('Robot_1', 'Robot_2', 'Robot_3');
box on
grid on
grid minor
set(gca, 'fontsize', 12);
